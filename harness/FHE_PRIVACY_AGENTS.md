...
- FHE: OpenFHE CKKS (`openfhe-python==1.5.1.0`, `vendor/wheels/` 로컬 휠)
...
```bash
./init.sh setup
uv run python -m fhe.scripts.init_client
uv run python -m fhe.scripts.decrypt_profile
./init.sh
```

## HARD 제약

- **MUST**: 비밀키는 로컬에만 둔다. 외부 에이전트/LLM/MCP client에 비밀키를 넘기지 않는다.
...
- **MUST**: 동형 연산은 공개 컨텍스트로 수행하고, 복호화는 비밀키 보유 런타임에서만 한다.
...
| 영역 | 파일 | 책임 |
|---|---|---|
| FHE client | `fhe/client/fhe_engine.py` | 키 생성, 암호화, 복호화, 공개/비밀 번들 |
| 직렬화 | `fhe/client/utils.py` | Base64 ↔ 암호문 bytes |
| 프로필 | `fhe/client/local_profile.py` | 암호문 profile.json 저장/로드 |
