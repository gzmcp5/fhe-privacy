...
FHE-Privacy가 최상위 제품과 설치 진입점이다. OpenShell은 FHE-Privacy 릴리스가 버전과 무결성을
고정하고 내부적으로 설치하거나 배포하는 sandbox runtime dependency다. 사용자는 secure mode에서
OpenShell CLI로 Hermes를 직접 실행하지 않는다.

제품 관계 설명: [`0. product-runtime-relationship.md`](0.%20product-runtime-relationship.md)
...
fhe-privacy install
  ...
  -> FHE-Privacy 서비스 설치
  -> Hermes workload image 준비

fhe-privacy run --agent hermes
  -> Secure Gateway session
  -> OpenShell sealed sandbox
  -> Hermes + stateless MCP Bridge
```

하나의 제품 설치 경험은 하나의 프로세스나 컨테이너를 의미하지 않는다. Secure Gateway, Privacy
Core, OpenShell Gateway, reveal component와 Hermes sandbox는 서로 다른 프로세스와 권한으로 배치한다.
...
| 플랫폼 | 목표 |
|---|---|
| macOS/Linux host | Python package + platform OpenFHE wheel + checksum-pinned OpenShell host package/service |
| Windows host | Python wrapper + portable/subprocess engine + ACL named pipe 검토 |
| Remote Kubernetes | 초기 비범위; 후속 umbrella chart가 digest-pinned OpenShell OCI chart/image를 별도 workload로 배포 |
...
개발 환경의 플랫폼별 재현 절차는 `versions.lock`, `tools/openfhe/README.md`와
`tools/openshell/README.md`를 기준으로 한다. OpenFHE native wheel은 기록된 commit에서 플랫폼별로
별도 빌드하고, OpenShell은 `tools/openshell/install.sh`가 고정된 release asset만 내려받아 SHA-256과
실행 버전을 확인한다. upstream checksum 확인과 FHE-Privacy 호환성 검증은 서로 다른 상태이며,
후자의 테스트 근거 없이 새 플랫폼을 validated로 표시하지 않는다.

## OpenShell release input

`versions.lock`가 FHE-Privacy 릴리스와 함께 검증된 OpenShell 입력을 고정한다. 로컬 설치는 공식
또는 FHE-Privacy fork의 미리 빌드된 host package를 사용한다. Kubernetes 배포는 별도 OpenShell
Gateway와 Supervisor OCI image 및 Helm chart를 사용한다. `latest`, `dev`와 mutable tag는 허용하지
않는다.

OpenShell에 FHE-Privacy용 변경이 아직 upstream되지 않았다면 FHE-Privacy release CI가 정확한 fork
commit에서 OpenShell binary/image를 빌드하고 서명·checksum/digest를 기록한다. upstream release가
동일한 capability를 제공하면 공식 artifact로 전환한다. 최종 사용자는 OpenShell을 소스에서 빌드하지
않는다.
