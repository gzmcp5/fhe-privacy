...
- Bridge와 Core 사이는 agent-safe relay/IPC만 허용한다.
- Gateway는 별도 host-only Core interface를 사용한다.
- Reveal Authority는 별도 local channel과 terminal sink만 사용한다.
...
workspace, network allowlist, Core relay와 rollback 계획을 먼저 보여준다.

## 로컬 상태 분리

```text
~/.fhe-privacy/
  keys/              # Reveal Authority만 접근
...
Agent workspace에는 위 경로를 포함하지 않는다. Agent MCP Bridge에는 agent-safe session capability만
전달하며 key/Vault root 경로를 환경변수로 주지 않는다.
...
- host-only/reveal Core endpoint 차단
- 필요한 agent-safe relay만 허용
