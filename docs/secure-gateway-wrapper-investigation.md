...
8. handle-only stdio MCP Bridge와 HTTPS/mTLS agent-safe Core endpoint를 사용할 수 있다.
9. canary 누출과 bypass negative test를 자동화할 수 있다.
10. connect/SSH, exec, sync, forward와 service exposure를 차단한 sealed sandbox를 사용할 수 있다.
11. policy revision, supervisor heartbeat와 Core lease를 fail-closed로 결합할 수 있다.
...
| Core transport | sandbox에서 agent-safe HTTPS endpoint만 도달 가능한가 |
| Bridge trust | Hermes가 Bridge capability를 복제해도 추가 권한을 얻지 않는가 |
| Management | connect/SSH, exec, sync, forward와 service exposure를 모두 거부하는가 |
| Lifecycle | stale policy, heartbeat 단절과 lease 만료 후 Core가 요청을 거부하는가 |
...
agent-safe HTTPS, sealed sandbox, pre-LLM ingress와 terminal egress를 실제 버전에서 검증한 후 조사 기록과 E2E test
...
- Hermes의 Bridge capability 복제 후 forbidden/cross-session operation 거부
- OpenShell connect/exec/sync/forward와 Hermes direct input 거부
- heartbeat 단절, stale policy와 lease 만료 후 Core operation 거부
