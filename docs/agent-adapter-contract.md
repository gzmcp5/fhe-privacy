...
```json
{
  ...
  "supportsAgentSafeCoreHttps": true,
  "supportsSealedSandbox": true,
...
| `supportsAgentSafeCoreHttps` | sandbox가 제한된 HTTPS/mTLS agent-safe Core interface에 연결 가능 |
| `supportsSealedSandbox` | connect/SSH, exec, sync, forward와 service exposure를 차단 가능 |
...
- agent-safe HTTPS endpoint만 host 방향으로 허용
- connect/SSH, 임의 exec, file sync, port forward와 service exposure 차단
...
- agent-safe HTTPS endpoint와 TLS/sandbox identity 방식
- sealed sandbox 접근 제어 방식
...
| connect/exec/sync/forward와 직접 input 차단 | pass/fail |
| policy revision, heartbeat와 Core lease 결합 | pass/fail |
