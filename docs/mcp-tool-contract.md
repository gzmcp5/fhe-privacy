...
정의한다. Gateway ingress와 Reveal Authority는 이 MCP tool 목록에 포함하지 않는다.
...
- 모든 tool은 session-scoped agent capability로 agent-safe Core interface에 중계된다.
...
PII masking/encryption은 Gateway의 host-only Core interface가 수행한다. Reveal은 terminal egress의
Reveal Authority channel에서만 수행한다.
