...
| `U` | Unsupported | text-only 초기 모드에서 안전하게 탐지할 수 없어 입력 자체를 거부 |
...
| `biometric_media` | 얼굴 이미지, 지문 이미지, 음성 원본 | `U` | text-only 범위 밖 | secure mode 입력 거부 |
...
지원하지 않는 언어와 text가 아닌 attachment·이미지·음성은 놓칠 수 있다. Secure mode는 지원하지 않는
입력 형식을 거부하며 모든 개인정보를 자동 탐지한다고 주장하지 않는다.
